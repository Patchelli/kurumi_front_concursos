...
export type PrivateMaterial = { id: number; topicId: number; name: string; mimeType: string; createdAt: string; studyLocation?: string | null };
...
const updateStudyLocation = (owner: PrivateMaterialOwner, id: number, materialId: number, studyLocation: string | null) => clientRequest<PrivateMaterial>({ url: `${ownerPath(owner, id)}/${materialId}/study-location`, method: HttpMethod.Put, body: { studyLocation } });

export const privateMaterialService = { listFiles, list, link, remove, file, updateStudyLocation };

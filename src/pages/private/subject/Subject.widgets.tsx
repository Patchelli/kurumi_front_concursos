...
function MaterialsPanel({ journeyId, knowledgeAreaId, nodeId, title, onList, onSave, onDelete, onClose, onLoaded }: {
  journeyId: number;
...
type SubtopicRowProps = { journeyId: number; child: SyllabusNodeResponse; studyState?: SyllabusNodeStudyResponse; onStartStudy(): void; onRemove(): void; onSaveStudy(request: SyllabusNodeStudyRequest): Promise<SyllabusNodeStudyResponse> };
function SubtopicRow({ journeyId, child, studyState, onStartStudy, onRemove, onSaveStudy }: SubtopicRowProps) {
...
export function TopicRow({ journeyId, knowledgeAreaId, topic, onAddSubtopic, onStartStudy, onRemove, onRemoveSubtopic, nodeStudy, onSaveNodeStudy, onListResources, onSaveResource, onDeleteResource }: TopicRowProps) {
  ...
  return (
    <div className={`sb-topic-card${open ? ' sb-open' : ''}${done ? ' sb-done' : ''}`}>
      ...
      {open && <div className="sb-topic-body">
        ...
        <div className="sb-subtopics-block">{topic.children.map(child => <SubtopicRow key={child.id} journeyId={journeyId} child={child} studyState={nodeStudy.find(item => item.syllabusNodeId === child.id)} onStartStudy={() => onStartStudy(child.id)} onRemove={() => onRemoveSubtopic(child.id)} onSaveStudy={onSaveNodeStudy} />)}<button className="sb-add-subtopic-btn" onClick={onAddSubtopic}><span>＋</span> adicionar subtópico</button></div>
      </div>}

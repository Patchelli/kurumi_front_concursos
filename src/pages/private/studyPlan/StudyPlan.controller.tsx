...
export function StudyPlanController() {
  ...
  const [questionNotebooks, setQuestionNotebooks] = useState<StudyResource[]>([]);
  useEffect(() => { if (!Number.isSafeInteger(journeyId) || journeyId <= 0) { setJourney(null); setLoading(false); return; } let active = true; journeyService.findById(journeyId).then(data => active && setJourney(data)).catch(error => active && (setJourney(null), toast.error(getRequestErrorMessage(error, 'Não foi possível carregar o plano de estudos.')))).finally(() => active && setLoading(false)); syllabusNodeStudyService.list(journeyId).then(data => active && setNodeStudy(data)).catch(() => active && setNodeStudy([])); studyResourceService.list(journeyId).then(data => active && setQuestionNotebooks(data.filter(item => Number(item.kind) === 3 || String(item.kind) === 'QuestionNotebook'))).catch(() => active && setQuestionNotebooks([])); studyRoutineService.findAll(journeyId).then(async routines => { const routine = routines.find(item => item.active) ?? routines[0]; if (!active || !routine) return; setRoutineId(routine.id); setConfiguration(routine.configuration); const now = new Date(); const end = new Date(now.getFullYear(), now.getMonth() + 13, 0); let data = await studyRoutineService.blocks(routine.id, localDate(startOfWeek(now)), localDate(end)); if (!data.length) data = await studyRoutineService.generate(routine.id, journeyId); if (active) setRoutineBlocks(data); }).catch(() => {}); return () => { active = false; }; }, [journeyId]);
  useEffect(() => {
  ...
  useEffect(() => {
    const refreshNotebooks = (event: Event) => {
      if ((event as CustomEvent<{ journeyId: number }>).detail?.journeyId !== journeyId) return;
      void studyResourceService.list(journeyId).then(items => setQuestionNotebooks(items.filter(item => Number(item.kind) === 3 || String(item.kind) === 'QuestionNotebook')));
    };
    window.addEventListener('question-notebooks-changed', refreshNotebooks);
    return () => window.removeEventListener('question-notebooks-changed', refreshNotebooks);
  }, [journeyId]);
  ...
  async function saveCognitivePairing(nodeId: number, value: string) {
    ...
    if (!area || !node) throw new Error('T�pico n�o encontrado.');

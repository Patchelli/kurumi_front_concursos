...
export function StudyPlanController() {
  ...
  async function completeBlock(blockId: number, completed: boolean, completedMinutes = 0, scheduleReview = false, reviewDate: string | null = null, clearPending = false) {
    ...
    const result = await studyRoutineService.complete({ blockId, completed, completedMinutes: completedMinutes > 0 ? completedMinutes : (completed ? planned : 0), scheduleReview, reviewDate, clearPending });
  ...
  return <StudyPlanView journey={journey} loading={loading} configuration={configuration} routineBlocks={routineBlocks} nodeStudy={nodeStudy} onSaveConfiguration={saveConfiguration} onCompleteBlock={completeBlock} onSaveNodeStudy={saveNodeStudy} onListResources={nodeId => studyResourceService.list(journeyId, nodeId)} onSaveResource={request => studyResourceService.register(request)} onDeleteResource={async resourceId => { await studyResourceService.remove(resourceId); }} onBack={() => navigate('/inicio')} onOverview={() => navigate(`/jornadas/${id}`)} onOpenContent={() => navigate(`/jornadas/${id}/materias`)} onOpenCapsule={() => navigate(`/jornadas/${id}/capsulas`)} onOpenSimulados={() => navigate(`/jornadas/${id}/simulados`)} onOpenSubject={areaId => navigate(`/jornadas/${id}/materias/${areaId}`)} />;
}

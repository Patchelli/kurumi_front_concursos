...
const colors = Object.fromEntries(Object.entries(calendarTokens.eventTypes).map(([key, value]) => [key, value.color])) as Record<ECalendarEventType, string>;
...
export default function CalendarView() {
  ...
  return <div className="journey-hub">
    ...
    <div className="agenda-page">
      <header className="agenda-header"><div><span className="agenda-eyebrow">ORGANIZAÇÃO</span><h1>Calendário</h1><p>Datas importantes dos seus concursos e da sua rotina de estudos.</p></div><button className="agenda-primary" onClick={() => openNew()}>＋ Novo evento</button></header>
      {loading ? <StudyLoading variant="section" label="Carregando calendário…" /> : <section className="agenda-layout">
        <div className="agenda-calendar"><div className="agenda-calendar-head"><button onClick={() => setCursor(new Date(year, month - 1, 1))}>‹</button><h2>{cursor.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</h2><button onClick={() => setCursor(new Date(year, month + 1, 1))}>›</button></div><div className="agenda-week">{['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map(day => <span key={day}>{day}</span>)}</div><div className="agenda-grid">{cells.map((day, index) => { const date = day ? iso(new Date(year, month, day)) : ''; const dayEvents = events.filter(item => item.date === date); return <button key={index} className={`agenda-day${date === iso(new Date()) ? ' today' : ''}${!day ? ' blank' : ''}`} disabled={!day} onClick={() => openNew(date)}><strong>{day || ''}</strong>{dayEvents.slice(0, 3).map(item => <i key={item.id} style={{ background: colors[item.type] }}>{item.title}</i>)}{dayEvents.length > 3 && <small>+{dayEvents.length - 3}</small>}</button>; })}</div></div>
        <aside className="agenda-list"><div className="agenda-list-head"><div><span className="agenda-eyebrow">ESTE MÊS</span><h2>Próximas datas</h2></div><span>{monthEvents.length}</span></div>{monthEvents.length ? monthEvents.map(item => <article className="agenda-event" key={item.id}><time>{new Date(`${item.date}T12:00:00`).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }).replace('.', '')}</time><div><b>{item.title}</b><small style={{ color: colors[item.type] }}>{labels[item.type]}{item.note ? ` · ${item.note}` : ''}</small></div><button className="agenda-edit" onClick={() => editEvent(item)} aria-label={`Editar ${item.title}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4L19 9a2.8 2.8 0 0 0-4-4L4 16v4Zm9.5-12.5 3 3" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg></button><button onClick={() => setDeleteTarget(item)}>×</button></article>) : <div className="agenda-empty"><span>□</span><b>Nenhuma data neste mês</b><p>Cadastre a prova, boleto, edital ou qualquer lembrete importante.</p><button onClick={() => openNew()}>Adicionar primeira data</button></div>}</aside>
      </section>}
    </div>
    {open && <div className="agenda-dialog-backdrop" onMouseDown={event => event.currentTarget === event.target && setOpen(false)}><form className="agenda-dialog" onSubmit={submit}><button type="button" className="agenda-close" onClick={() => setOpen(false)}>×</button><span className="agenda-eyebrow">{draft.id !== null ? 'EDITAR DATA' : 'NOVA DATA'}</span><h2>{draft.id !== null ? 'Editar evento' : 'Adicionar evento'}</h2><label>Título<input autoFocus value={draft.title} onChange={event => setDraft(current => ({ ...current, title: event.target.value }))} placeholder="Ex.: Prova SEFAZ" /></label><div className="agenda-form-row"><label>Data<input type="date" value={draft.date} onChange={event => setDraft(current => ({ ...current, date: event.target.value }))} /></label><label>Tipo<select value={draft.type} onChange={event => setDraft(current => ({ ...current, type: event.target.value as ECalendarEventType }))}>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><label>Observação (opcional)<textarea value={draft.note} onChange={event => setDraft(current => ({ ...current, note: event.target.value }))} placeholder="Ex.: pagar até 18h" /></label><button className="agenda-primary" type="submit" disabled={saving}>{saving ? 'Salvando…' : 'Salvar evento'}</button></form></div>}

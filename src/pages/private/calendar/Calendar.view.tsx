...
export default function CalendarView() {
  ...
  const [questionTarget, setQuestionTarget] = useState<CalendarEvent | null>(null);
  ...
  return <div className="journey-hub">
    <AppHubNavigation active="calendar" firstName={firstName} onLogout={logoutMethod} />
    <div className={t.page}>
      <header className={t.header}><div><span className={t.eyebrow}>ORGANIZAÇÃO</span><h1 className={t.title}>Calendário</h1><p className={t.subtitle}>Anote provas, boletos, editais e lembretes importantes.</p></div><button className="cp-new-btn" type="button" onClick={() => openNew()}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>Novo evento</button></header>
    ...
    {questionTarget?.journeyId && questionTarget.knowledgeAreaId && questionTarget.syllabusNodeId && <QuestionRegisterDialog journeyId={questionTarget.journeyId} knowledgeAreaId={questionTarget.knowledgeAreaId} syllabusNodeId={questionTarget.syllabusNodeId} title={questionTarget.title.replace(/^Questões:\s*/, '')} onClose={() => setQuestionTarget(null)} onSaved={() => setEvents(current => current.filter(item => item.id !== questionTarget.id))} />}
  </div>;
}

...
export function ContestJsonImport({ onImport }: Props) {
  ...
  function load(content: string) { setValue(content); try { const subjects = parse(content); onImport(subjects); setMessage(`${subjects.length} disciplinas importadas e combinadas.`); } catch (error) { setMessage(error instanceof Error ? error.message : 'JSON inválido.'); } }
  async function file(selected?: File) { if (selected) load(await selected.text()); }
  return <section className="contest-json-import"><header><div><span>⇩</span><div><strong>Importar conteúdo</strong><small>O conteúdo será somado ao que já foi informado.</small></div></div><label>Importar JSON<input type="file" accept=".json,application/json" onChange={event => file(event.target.files?.[0])}/></label></header><textarea value={value} onChange={event => load(event.target.value)} placeholder={'{\n  "materias": [{ "nome": "Português", "topicos": [] }]\n}'}/>{message && <p className={message.includes('combinadas') ? 'success' : 'error'}>{message}</p>}</section>;
}
